...
public class BarrelBlockClientExtensions implements IClientBlockExtensions {
	...
	public boolean addHitEffects(BlockState state, Level level, HitResult target, ParticleEngine manager) {
		...
		if (state.getRenderShape() != RenderShape.INVISIBLE) {
			...
			manager.add((new CustomTintTerrainParticle(clientLevel, d0, d1, d2, 0.0D, 0.0D, 0.0D, state, pos).sophisticatedFabricLibrary_updateSprite(state, pos)).setPower(0.2F).scale(0.6F));
		}
			...
			for (int l = 0; l < i; ++l) {
				for (int i1 = 0; i1 < j; ++i1) {
					for (int j1 = 0; j1 < k; ++j1) {
						...
						manager.add(new CustomTintTerrainParticle(clientLevel, pos.getX() + d7, pos.getY() + d8, pos.getZ() + d9, d4 - 0.5D, d5 - 0.5D, d6 - 0.5D, state, pos).sophisticatedFabricLibrary_updateSprite(state, pos));
					}
				}
			}
		});
