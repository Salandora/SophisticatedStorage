...
public class DecorationTableMenu extends AbstractContainerMenu implements ISyncedContainer {
	...
	public DecorationTableMenu(int containerId, Player player, BlockPos pos) {
		...
		blockEntity = player.level().getBlockEntity(pos, ModBlocks.DECORATION_TABLE_BLOCK_ENTITY_TYPE).orElse(null);
		if (blockEntity == null) {
	...
	private int addDecorationSlots() {
		...
		addSlot(new SlotItemHandler(dyes, 0, x, yOffset).sophisticatedFabricLibrary_setBackground(InventoryMenu.BLOCK_ATLAS, EMPTY_RED_DYE_SLOT_BACKGROUND));
		x += 18;
		addSlot(new SlotItemHandler(dyes, 1, x, yOffset).sophisticatedFabricLibrary_setBackground(InventoryMenu.BLOCK_ATLAS, EMPTY_GREEN_DYE_SLOT_BACKGROUND));
		x += 18;
		addSlot(new SlotItemHandler(dyes, 2, x, yOffset).sophisticatedFabricLibrary_setBackground(InventoryMenu.BLOCK_ATLAS, EMPTY_BLUE_DYE_SLOT_BACKGROUND));
	...
	private int addDecorationSlot(ItemStackHandler itemHandler, int slotIndex, int xOffset, int y, int yPadding) {
		...
		}.sophisticatedFabricLibrary_setBackground(InventoryMenu.BLOCK_ATLAS, EMPTY_MATERIAL_SLOT_BACKGROUND));
		y += 18;
		y += yPadding;
		return y;
	}
