...
public class PaintbrushOverlay {
	...
	public static final HudRenderCallback HUD_PAINTBRUSH_INFO = (guiGraphics, deltaTracker) -> {
		...
		if (!(level.getBlockState(pos).getBlock() instanceof StorageBlockBase) && level.getBlockState(pos).getBlock() != ModBlocks.CONTROLLER) {
			return;
		}

		InventoryHelper.getItemFromEitherHand(player, ModItems.PAINTBRUSH).flatMap(paintbrush -> getItemRequirementsFor(paintbrush, player, level, pos))
				.ifPresent(itemRequirements -> {
					...
					guiGraphics.drawString(font, missingItems, x + 1, y, DyeColor.WHITE.getTextColor(), true);
